...
def test_autoincrement_translated_in_create_table():
    ...
    assert translate_sql(sql) == "CREATE TABLE t (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, v TEXT)"
...
def test_placeholder_autoincrement_and_datetime_now_together():
    ...
    expected = ("CREATE TABLE t (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "
...
def test_ddl_idioms_idempotent_on_already_postgres_sql():
    # Arbitrary already-Postgres identity DDL (deliberately GENERATED ALWAYS
    # here, independent of what our own AUTOINCREMENT translation emits) must
    # pass through untouched -- the regex only matches the SQLite idiom.
...
def test_autoincrement_as_string_literal_data_is_also_translated_known_risk():
    ...
    expected = "INSERT INTO t (note) VALUES ('id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY')"
    assert translate_sql(sql) == expected
...
# ---------------------------------------------------------------------------
# P05.5 T2: AUTOINCREMENT -> GENERATED BY DEFAULT AS IDENTITY, live-PG proof
# ---------------------------------------------------------------------------

@pytest.mark.skipif(not pg, reason="PG_DSN not set")
def test_autoincrement_identity_accepts_explicit_id_and_still_autogenerates(monkeypatch):
    # BY DEFAULT (not ALWAYS) must accept an explicit id INSERT -- the data
    # migration copies original SQLite ids to preserve FK relationships, which
    # GENERATED ALWAYS AS IDENTITY would reject outright.
    monkeypatch.setenv("DB_BACKEND", "postgres")
    cx = db.connect("ignored")
    cx.execute("DROP TABLE IF EXISTS t2_identity")
    cx.execute(translate_sql("CREATE TABLE t2_identity (id INTEGER PRIMARY KEY AUTOINCREMENT, v TEXT)"))
    cx.commit()

    # (a) explicit-id insert -- FAILS under GENERATED ALWAYS AS IDENTITY.
    cx.execute("INSERT INTO t2_identity (id, v) VALUES (5, 'x')")
    cx.commit()
    row = cx.execute("SELECT id, v FROM t2_identity WHERE id=5").fetchone()
    assert row[0] == 5 and row["v"] == "x"

    # (b) auto-gen insert still works for the app's normal RETURNING-id path.
    gen_row = cx.execute("INSERT INTO t2_identity (v) VALUES ('y') RETURNING id").fetchone()
    cx.commit()
    assert gen_row[0] is not None and gen_row[0] != 5
