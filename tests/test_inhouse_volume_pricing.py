...
def test_ff_unit_cents_by_total_qty():
    ...
    # effective = round(6997 * (1 - volume_pct(total)/100)), floored at list*0.57
    assert f(FF, 1, s) == 6997          # vp(1)=0%
    assert f(FF, 3, s) == 6450          # vp(3)≈7.82%
    assert f(FF, 6, s) == 5629          # vp(6)≈19.55%
    assert f(FF, 12, s) == 3988         # vp(12)=43% (== floor)
    assert f(FF, 99, s) == 3988         # capped at the 12-unit max
    assert f(NONFF, 12, s) == 7000      # non-FF: list price, no volume
...
def test_multi_ff_lines_share_total_rate():
    appmod = _app()
    s = _pricing.load_settings(None)
    # total FF qty 6 → BOTH FF lines priced at the vp(6) rate, even a qty-2 line
    assert appmod._inhouse_ff_unit_cents(FF, 6, s) == 5629
    assert appmod._inhouse_ff_unit_cents(FF2, 6, s) == 5629


def test_line_unit_override_wins():
    appmod = _app()
    s = _pricing.load_settings(None)
    assert appmod._inhouse_line_unit_cents(FF, 5000, 12, s) == 5000   # explicit override
    assert appmod._inhouse_line_unit_cents(FF, None, 12, s) == 3988   # FF priced
