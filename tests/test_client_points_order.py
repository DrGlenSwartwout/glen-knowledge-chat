...
points (Task 2): fee-capped redemption applied as an invoice discount.

Stubs mirror test_client_order.py:
  - _retail_for -> 7000
  - practitioner_price_for -> 7000 (fixed S; quote_line runs for real)
  - qb.find_or_create_customer -> {"Id": ...}
  - qb.create_invoice -> captures kwargs (incl. discount_cents)
...
def _common_stubs(monkeypatch, cap):
    ...
    monkeypatch.setattr(dc.qb, "create_invoice",
        lambda cust, lines, **k: cap.update(
            cust=cust, lines=lines, discount_cents=k.get("discount_cents")
        ) or {"Id": "INV", "TotalAmt": 70.0})
...
def test_large_redemption_capped_at_total_fee(monkeypatch):
    ...
    cap0 = {}
    _common_stubs(monkeypatch, cap0)
    ...
    assert cap["discount_cents"] == total_fee
