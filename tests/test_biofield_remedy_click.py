...
def test_remedy_click_records_biofield(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(app_module, "_biofield_visible_slugs", lambda row, email: {"neuro-magnesium"})
...
def test_remedy_click_rejects_slug_not_in_reveal(monkeypatch, tmp_path):
    db = str(tmp_path / "log.db")
    cx = sqlite3.connect(db); re.init_recommendation_events(cx); cx.close()
    monkeypatch.setattr(app_module, "LOG_DB", db, raising=False)
    monkeypatch.setattr(app_module, "_biofield_verify_token", lambda th: (True, {"id": 1, "email": "a@b.com"}))
    ...
    app_module.app.config["TESTING"] = True
    c = app_module.app.test_client()
    r = c.post("/begin/biofield/anytok/remedy-click", json={"slug": "not-in-reveal"})
    assert r.get_json()["ok"] is True
    cx = sqlite3.connect(db)
    assert not any(e["source_key"] == "biofield" for e in re.list_events(cx, "a@b.com"))
