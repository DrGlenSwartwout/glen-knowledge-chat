#!/usr/bin/env python3
"""Daily Supabase practitioners → People hub tag sync cron entry point.

Runs on Render's cron worker. Posts to the web service's
/admin/sync-practitioner-tags endpoint, which reads the Supabase practitioners
table and upserts each into the SQLite `people` table tagged
type:practitioner / type:practitioner-cold + consent (additive). Keeps the
People hub fresh as the directory grows and practitioners register.

Same cross-container pattern as run_pb_tag_sync_cron.py — the persistent disk +
Supabase credentials live on the web service, not the cron container.
...
def main():
    url = f"{WEB_URL}/admin/sync-practitioner-tags"
        ...
        with urllib.request.urlopen(req, timeout=600) as resp:
            ...
            try:
                ...
                if data.get("ok"):
                    s = data.get("summary", {})
                    print(
                        f"Practitioner sync: {s.get('rows_fetched',0)} rows, "
                        f"{s.get('people_upserted',0)} upserted, "
                        f"{s.get('engaged',0)} engaged, "
                        f"{s.get('cold',0)} cold, "
                        f"{s.get('skipped_no_email',0)} skipped, "
