...
# --days 1: only draft shipments from the last day (going-forward). The shipments
# table makes re-runs idempotent, so the 15-min cadence never double-drafts.
exec doppler run -p remedy-match -c prd -- \
  python3 cns_tracking_watcher.py --live --days 1 \
