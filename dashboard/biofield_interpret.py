...
_SYSTEM = (
    ...
    "- PSYCHOEMOTIONAL / ESSENCE STRESS: 'psycho[-]emotional essence stress ... the stress is "
    "called [NAME]' (or 'the stress is [NAME]') -> the NAMED essence [NAME] is that layer's `head` "
    "(the life-stress resonance). Do NOT use the generic phrase 'psycho emotional essence stress' as "
    "the head when a name is given. The remedy that follows ('the remedy is [X]' / 'and that's [X]') "
    "is a DIFFERENT essence -> that layer's `remedy` (in Terrain Restore). Never collapse the head "
    "essence and the remedy essence into one.\n"
    ...
    "- DOSE FIELDS ARE VERBATIM-ONLY. NEVER infer, complete, or standardize a dose field that was "
    "not spoken. Fill ONLY the fields the clinician actually said and leave every field that was "
    "not spoken as an empty string — a blank field is filled from the product's catalog dosing "
    "downstream, and a guess silently overrides it. If he says only 'Fiber Cleanse, one a day', set "
    "frequency='one a day' and leave dosage AND timing as ''. Do not add 'with food', '1 capsule', "
    "'between meals', or any other plausible default.\n"
