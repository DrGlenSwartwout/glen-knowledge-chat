...
#
# 1) `INTEGER PRIMARY KEY AUTOINCREMENT` -> `BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY`
#    (BY DEFAULT, not ALWAYS: the data migration must INSERT explicit original
#    ids to preserve FK relationships, which `GENERATED ALWAYS AS IDENTITY`
#    rejects. `BY DEFAULT` accepts explicit ids and still auto-generates for
#    the app's normal `INSERT ... RETURNING id` path. The identity sequence
#    must be reset after the migration's explicit inserts — handled separately.)
...
def _translate_ddl_idioms(sql: str) -> str:
    ...
    if _RE_PRAGMA_FOREIGN_KEYS.match(sql):
        return "SELECT 1"
    sql = _RE_AUTOINCREMENT.sub("BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY", sql)
