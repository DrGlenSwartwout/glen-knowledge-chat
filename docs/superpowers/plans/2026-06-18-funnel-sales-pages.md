...
- **Phase 3 — Image generation via local watcher.** Queue table + `sales_page_watcher.py`; seed **2 variants per type** (botanical + mechanism = 4 images) written back; page backfills; plus a **challenger-regeneration** trigger (render a new variant when a pair has a champion + isn't converged).
- **Phase 4 — Image feedback + credit + per-viewer memory.** `sales_page_viewers` table; **pairwise forced-choice picks** (favorite of each within-type pair, + "neither" escape) with a **champion-challenger** win/loss tally and significance/convergence/cadence guardrails; `points.credit` + `_settle_order_points` gating; section-open memory.
...
**Phase 3 — image generation via local watcher.** `sales_page_gen_queue` table + `process_queue.enqueue` pattern; `sales_page_watcher.py` (launchd) renders the **seed 2 variants per type** (4 Flux images) via the local image-studio infra (Replicate, `deploy-chat311` venv), writes to `static/media/sales/<slug>/` + updates the draft record; page polls/backfills. A separate enqueue path renders a **single challenger** when Phase 4 flags a pair for replacement. Keep all Replicate calls off Render web workers.

**Phase 4 — image feedback + credit + memory.** `sales_page_viewers` table (`person_id, product_slug, sections_viewed, pair_picks_json, credit_earned`) plus a per-variant `sales_page_image_stats` tally (`product_slug, type, variant_id, wins, losses, state[champion|challenger|retired]`). Endpoints to record a **pair pick** (or "neither") and section-open state (extend the existing open-sections write). On a pick in **both pairs** → `points.credit(cx, email, value_cents=100, reason="page_pick_images", order_ref=f"page_{slug}")`; realize inside `_settle_order_points` gated on the order containing that slug; idempotent via `points.has_entry`. **Champion-challenger logic:** declare a champion only past a significance bar (min sample + win-rate confidence); on a clear winner, flag the loser for a challenger render (Phase 3); stop after a champion beats K successive challengers (convergence cap) and at most one render per pair per N days (cadence cap).
