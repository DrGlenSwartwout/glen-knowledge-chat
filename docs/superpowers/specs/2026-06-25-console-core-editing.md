...
- A **"unlock"** action calls `unlock_core_field(...)` → removes the field from `overrides` (the value is left as-is; the next FMP import, if any, will overwrite it). "Unlock" re-opens the field to FMP; it does NOT restore a prior value (we don't keep a shadow copy — YAGNI).
...
- Editable core fields:
  - `ingredients`: `name`, `form`, `common_names`, `par_level`, `par_level_unit`
  - `ingredient_sources`: `price_per_unit`, `unit_size`, `unit_type`
  - `formulation_items`: `dose`, `dose_unit`

  `common_names` is stored as a JSON array; the console edits it as a comma-separated list (UI splits/joins on `, `, the module serializes to/from JSON). Override protection applies to it like any other column.
- Module functions `set_core_field` / `unlock_core_field` with a per-table **allowlist** of editable core fields (mirrors the existing `_update_allowed` curated pattern).
- `/api/*` endpoints for core-field PATCH + unlock.
- Console UI: turn those read-only fields into editable inputs in the Ingredients, Suppliers/Sources, and Formulations tabs, each with an **"overridden ⟳ unlock"** indicator.
- Tests proving: edit writes value + override; re-import skips overridden fields on that row but refreshes others and other rows; unlock re-opens a field; par_level column + backfill + readers.
...
5. **`app.py`** — endpoints (all `@require_console_key`, `ok`/`fail`):
   ...
   - `POST /api/<entity>/<int:id>/unlock` — body `{field}` → `unlock_core_field`.
...
7. **Tests** (new + extended):
   ...
   - route tests (Pinecone-skip) for the core PATCH + unlock endpoints.
...
3. Edit a source `price_per_unit` → reorder cost reflects it; unlock → field re-opens to FMP (next import would refresh).
