...
- **Content model = the sales-page model:** AI proposes the narrative + the two scores + the related-forms list; Glen edits/approves in the console; draft banner until approved; generate-on-view (lazy).
...
```
Functions mirror `sales_pages.py`: `init_table`, `get_section`/`upsert_section`, `get_page`, `set_state`, plus `set_scores(cx, slug, research, traditional)` and `set_related_forms(cx, slug, forms)`. Getters use a per-cursor Row factory.
...
- `propose_ratings_and_forms(ingredient) -> {research_score, traditional_score, related_forms}`: a synchronous AI call (haiku) that proposes the two 1-10 scores (research = strength/volume of the studies found; traditional = historical/traditional-use evidence) and a related-forms list (other forms of the same nutrient with a superior/inferior/comparable verdict and a one-line note), each form slugged via `ingredients.slugify`. These are PROPOSALS; Glen edits/approves. Returns safe defaults (scores null, forms []) on failure.
...
- `GET /begin/ingredient-page-data/<slug>` (mirror `begin_product_page_data`): returns `{slug, name, sections[], research_score, traditional_score, related_forms, formulations[], standalone_product_slug|null, ai_state}`. On first request for a slug with no row, it triggers (best-effort, in-process) `propose_ratings_and_forms` to seed the scores + forms as a draft, and marks the narrative sections `ai:"pending"` so the page streams them. `ai_state` drives the draft banner (suppressed when `state=approved`).
...
4. **Related forms** (each `{name, verdict, note}` with a superior/inferior badge; the name links to `/begin/ingredient/<slug>` in a new tab).
5. **In these formulations** (each formulation links to `/begin/product/<slug>`).
...
- `ingredient_page.edit` - save edited section text AND/OR the two scores AND/OR the related-forms list (stays draft).
- `ingredient_page.approve` - set state `approved` (drops the draft banner).
- `ingredient_page.regenerate` - re-run the narrative + `propose_ratings_and_forms` for review (stays draft).
A new `static/console-ingredient-pages.html` (model `console-sales-pages.html`): edit the two narrative sections, the two 1-10 scores, and the related-forms list; approve. A "Ingredient Pages" console nav sub-tab.
...
2. The page calls `/begin/ingredient-page-data/<slug>` -> resolver returns details + formulations + study list; if no stored row, `propose_ratings_and_forms` seeds the two scores + related-forms (draft) and the narrative sections stream via the gen endpoint.
...
- `propose_ratings_and_forms` / narrative gen failures -> safe defaults (null scores hide the gauges, empty forms hide that section); never 500.
...
- **Storage:** `ingredient_pages` init + `upsert_section`/`get_section`, `set_scores` (clamped 1-10), `set_related_forms`, `set_state` draft->approved, per-cursor Row factory (no connection leak).
- **Gen + page-data:** `/begin/ingredient-page-data/<slug>` returns the expected keys incl. `research_score`/`traditional_score`/`related_forms`/`formulations`; `ai_state` reflects state; the gen SSE caches and replays.
...
2. **Increment 2 - AI narrative + ratings + related forms:** `ingredient_copy.py`, `ingredient_pages.py` store, the gen SSE endpoint, `propose_ratings_and_forms`, the two gauges + related-forms rendering, the draft banner.
