...
@app.route("/api/qbo/recurring-probe", methods=["POST"])
def qbo_recurring_probe():
    """Console-gated schema probe: posts a recurring Invoice template using the
    RecurringInfo from the request body, returns Intuit's exact response, and deletes
    anything it creates. Lets us find the accepted RecurringTransaction shape without a
    redeploy per attempt. Body: {"recurring_info": {...}, "bill_email": bool}."""
    if not _qbo_auth_ok():
        return jsonify({"error": "Unauthorized"}), 401
    data = request.get_json(silent=True) or {}
    try:
        ...
        inv = {
            "Line": [{"DetailType": "SalesItemLineDetail", "Amount": 149.00,
                      ...
                      "SalesItemLineDetail": {"ItemRef": {"value": item["Id"]},
                                              "Qty": 1, "UnitPrice": 149.00}}],
            "CustomerRef": {"value": cust["Id"]},
            "RecurringInfo": rinfo,
        }
        if data.get("bill_email"):
            inv["BillEmail"] = {"Address": "zztest+groupcoaching@example.com"}
        try:
            out = qb._post("/recurringtransaction", {"RecurringTransaction": {"Invoice": inv}})
        except Exception as pe:
            return jsonify({"ok": False, "sent": rinfo, "error": str(pe)}), 200
        created = (out.get("RecurringTransaction") or {}).get("Invoice") or out.get("Invoice") or {}
        rid, stok = created.get("Id"), created.get("SyncToken")
        deleted = None
        if rid:
            try:
                ...
                deleted = f"DELETE FAILED ({de}) — clean up id {rid} manually"
        return jsonify({"ok": True, "created_id": rid, "deleted": deleted,
                        "recurring_info_echo": created.get("RecurringInfo")})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
